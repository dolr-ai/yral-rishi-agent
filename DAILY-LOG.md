...
**Next blocker surfaced + being fixed here:** the avatar-upload step then failed 422 `missing field delegated_identity_wire` — Prakash's `storage-interface.prakash.yral.com/api/v1/user/profile-image` still demands the old IC auth (Prakash left; service orphaned). Decision: **don't migrate Prakash's Rust service for this — add the one endpoint to our own agent service.** This PR adds `POST /api/v1/user/profile-image` (JWT auth via PR #489 pattern; no `delegated_identity_wire`, no IC canister write — SpacetimeDB already owns the profile URL via `update_profile_details_v2`). Bytes land in a **dedicated public Hetzner bucket** `yral-profile-pictures` (hel1, public-read policy, reuses the WAL-G Hetzner creds) → durable public URL. Bucket + image/jpeg serving **verified against the live bucket**. Scope = **bot avatars only** for now; human profile pics + video-gen are the later storage-service migration ([dolr-ai/prakash-videogen#5](https://github.com/dolr-ai/prakash-videogen/issues/5) tracks the bot-pfp-bucket sequencing).
