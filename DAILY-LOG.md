...
**Next blocker surfaced + being fixed here:** the avatar-upload step then failed 422 `missing field delegated_identity_wire` — Prakash's `storage-interface.prakash.yral.com/api/v1/user/profile-image` still demands the old IC auth (Prakash left; service orphaned). Decision: **don't migrate Prakash's Rust service for this — add the one endpoint to our own agent service.** This PR adds `POST /api/v1/user/profile-image` (JWT auth via PR #489 pattern; no `delegated_identity_wire`, no IC canister write — SpacetimeDB already owns the profile URL via `update_profile_details_v2`). Bytes land in a **dedicated Storj bucket** `yral-profile-pictures` served publicly via a read-only, no-expiry **link-share** → durable public URL. Chose Storj over Hetzner deliberately: it reuses the agent's existing Storj creds (chat media), so the public-facing service never gets the Hetzner WAL-G creds that can also reach the DB-backup bucket — cleaner credential isolation. Upload + link-share serving **verified end-to-end** (HTTP 200, image/jpeg). Scope = **bot avatars only** for now; human profile pics + video-gen are the later storage-service migration ([dolr-ai/prakash-videogen#5](https://github.com/dolr-ai/prakash-videogen/issues/5) tracks the bot-pfp-bucket sequencing).

**Deploy note:** the agent service needs `PROFILE_PIC_S3_ACCESS_KEY_ID` / `_SECRET_ACCESS_KEY` set to the Hetzner creds (same as WAL-G). After deploy, a one-line mobile repoint (bot-avatar upload host → `agent.rishi.yral.com`) + Rishi device test closes create-influencer fully.
