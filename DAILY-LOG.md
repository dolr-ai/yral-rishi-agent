...
**In PR — Wave 1 PR6 (#___): the real-database safety net (harness + marquee proof).** The payoff of the wave — real integration tests against a real Postgres, which the mocked-pool suite could never do. Shipped as the harness + 5 core tests; access-control / SSE / ETL tests follow as PR6b on the same harness.
