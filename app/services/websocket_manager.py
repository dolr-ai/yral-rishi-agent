...
async def _send_to_user(user_id: str, message: str):
    ...
    if dead_connections:
        async with _lock:
            if user_id in _connections:
                _connections[user_id] = [ws for ws in _connections[user_id] if ws not in dead_connections]
                if not _connections[user_id]:
                    del _connections[user_id]


async def broadcast_new_message(
    user_id: str, conversation_id: str, message: dict,
    influencer: dict, unread_count: int,
):
    event = json.dumps({
        "event": "new_message",
        "data": {
...
async def broadcast_conversation_read(user_id: str, conversation_id: str, read_at: str):
    event = json.dumps({
        ...
        "data": {"conversation_id": conversation_id, "unread_count": 0, "read_at": read_at}
    })
    await _send_to_user(user_id, event)


async def broadcast_typing_status(
    user_id: str, conversation_id: str, influencer_id: str, is_typing: bool,
):
    event = json.dumps({
        "event": "typing_status",
        "data": {
