...
- Add an integration test that spins up real public-api + a **contract-stub/fake user-memory HTTP server** (NOT the real user-memory-service testcontainer — see boundary note below) on an **ephemeral port** (pytest fixture binds to port 0; OS assigns a free port; fixture exposes the generated base URL and injects it as `USER_MEMORY_SERVICE_BASE_URL` into public-api's config for the test run). Tests then issue N=500 `POST /api/v1/chat/conversations/{id}/messages` calls (the per-request `influencer_id` lookup path once PR #141 lands).
