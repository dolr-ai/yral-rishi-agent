...
- **Session 1 deliverable note**: When choosing MOCK_LLM_FIXED_LATENCY_MS, query Langfuse traces of chat-ai to get the typical p95 LLM-call contribution (likely 50-200ms for Gemini Flash / 100-400ms for OpenRouter routing paths). Pick the median of those, OR the lower value to be conservative (a lower mock = stricter gate). Document the chosen value's source in the Gate A2-PR workflow file's comment block so future maintainers know what it approximates.
