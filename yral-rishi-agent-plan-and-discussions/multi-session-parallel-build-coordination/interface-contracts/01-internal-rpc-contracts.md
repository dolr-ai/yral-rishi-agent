...
- Path filters (expanded per Codex round-15 CONCERN — hot-path latency can regress from ANY of these surfaces, not just the directly-named service folders):
  ...
  - `yral-rishi-agent-conversation-turn-orchestrator/app/**` (orchestrator's full app code — `run_turn.py`, llm clients, soul-file composer, prompt assembly, all latency-affecting; Codex round-20 BLOCKER correctly flagged round-19's narrow `run_turn.py`-only filter as missing latency surface area)
  ...
  - `**/shared-config.yaml` (per-service shared-config.yaml files — each service has its own copy in its folder; timeouts, pool sizes, sentinel master name — all latency-affecting config. Codex round-20 BLOCKER correctly flagged round-19's `yral-rishi-agent-plan-and-discussions/shared-config.yaml` as a non-existent path — the real shape is per-service files matching `**/shared-config.yaml`.)
...
**Gate A2-PR explicit scope (clarifying Codex round-18 CONCERN)**: this gate validates **v2 infrastructure cost ≤ chat-ai infrastructure cost** by holding the LLM contribution CONSTANT (via mock) on the v2 side. It does NOT validate v2's real-world provider-routing latency — that gap is now closed by Gate A2-PROVIDER-PR below (Codex round-24 BLOCKER, correctly flagged). The 4-gate scope split:
  ...
  - **Gate A2-PROVIDER-PR** = "v2 real-LLM round-trip ≤ chat-ai full baseline" (real LLM provider; merge-blocking per E1; fires ONLY on PRs touching LLM-routing/client/model files — see filter below). Added round-25 to close the round-24 BLOCKER gap: a PR changing provider routing / model selection / llm_client behavior would not trigger Gate A2-PR (mock LLM neutralizes provider differences); Gate A2-PROVIDER-PR catches that class of change with real-provider measurement. **J2 zero-flake mitigation (Codex round-26 CONCERN)**: real-LLM measurements have inherent variance from provider latency, outages, network, and rate limits. To satisfy J2 without weakening E1, the gate computes p95 over N=500 calls (variance smooths out at this sample size for non-pathological provider state) + uses circuit-breaker semantics: if >5% of calls fail with provider-side errors (5xx / 429 / timeout) the workflow exits NEUTRAL (skip, not fail) with a "provider-degraded — re-run when resolved" message + Google Chat webhook alert to coordinator. Only INFRA p95 regressions (vs chat-ai baseline) cause hard FAIL. Provider-side flakes get an explicit "retry-or-investigate" exit code that branch protection treats as "not yet passed," not "failed." This makes real-LLM gating J2-compliant.
...
**Gate A2-PROVIDER-PR file filter + ownership split** (same two-part split as Gate A2-PR per round-13 — Session 1 owns the SCRIPT, coordinator owns the WORKFLOW FILE; Codex round-26 BLOCKER correctly flagged round-25's "Session 1 implements a separate workflow file" wording as a scope violation since `.github/workflows/**` is coordinator-owned):

  *Session 1 deliverable*: the real-LLM benchmark script in `yral-rishi-agent-plan-and-discussions/latency-baseline-capture-from-live-services-the-numbers-v2-must-beat/scripts/` — mirrors the Gate A2-PR script's structure but uses real Gemini/OpenRouter calls + the J2 circuit-breaker semantics described above.

  *Coordinator deliverable*: the workflow file in `.github/workflows/gate-a2-provider-pull-request-benchmark.yml` wiring the Session 1 script into a pull_request trigger with the path filter:
