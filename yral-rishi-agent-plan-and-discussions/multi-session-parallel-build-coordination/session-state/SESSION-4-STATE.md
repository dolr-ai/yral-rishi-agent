...
> Updated: 2026-05-23 (PR-B1 round-2 fixup pushed to PR #131 — adds Pydantic `Field(default=None, min_length=1)` on `RunTurnRequest.influencer_id` + new regression test `test_run_turn_real_llm_path_rejects_empty_string_influencer_id_request` defending against silent env-fallback on explicit `""` request bodies; closes Codex round-1 CONCERN; DRAFT stays on).
...
**PR #142 (PR-D1 Chunk A) is in Codex re-review; Chunk B is PAUSED-FOR-FRESH-SESSION.** Codex workflow on the initial Chunk A push hit a 401 Unauthorized at the comment-post step (workflow-token bug, not a code issue); actual verdict was `request_changes` per the workflow log but the comment never posted. This STATE update commit retriggers the workflow so the real verdict surfaces. After Codex re-fires and the verdict is visible: if APPROVE → coordinator launches a fresh session for Chunk B; if BLOCKER → Chunk A round-2 first in a fresh session, THEN Chunk B.

**Chunk B scope (for the fresh-session resume)**: wire `init_pool` + `close_pool` into `app/main.py`'s lifespan; add 3 FastAPI route handlers (`GET /v1/influencers?limit&offset` → `list[InfluencerMetadata]`, `GET /v1/influencers/{id}` → `InfluencerMetadata | 404`, `GET /v1/influencers/trending?limit` → `list[InfluencerMetadata]`); enforce pagination bounds per Q4 (limit default 20 min 1 max 100; offset default 0 min 0); endpoint-level role-comments documenting the contract ratification of DEP-013. ~300-400 lines per the original estimate.

**Chunk C scope (later)**: testcontainers-Postgres repository tests covering all 3 read methods + the Alembic round-trip test + endpoint tests covering pagination bounds + 404 path + trending ordering.

**Parallel work**: PR #136 (REDIS_PASSWORD wiring) now at 4 commits (round-4 regenerates `.env.example` per D8). PR-B1 round-2 fixup pushed to PR #131 — DRAFT stays on, awaiting Codex re-review. Round-2 adds `Field(default=None, min_length=1)` on `RunTurnRequest.influencer_id` + a new regression test for explicit-blank rejection (paired with the round-1 precedence test). Cumulative line count vs origin/main verified pre-push via `git diff --stat origin/main...HEAD`. Still **NOT I14 auto-merge eligible** (behavior-changing Python — adds optional model field on `RunTurnRequest` + resolver branch in `_generate_real_llm_reply` + observability marker + 2 paired tests now); coordinator manually merges via `gh pr merge 131 --squash` after Codex APPROVE. Queued next: **influencer-directory metadata schema + RPC endpoints** (Postgres `influencer_metadata` table, ETL from chat-ai's 3,941 `ai_influencers` rows, 3 endpoints for public-api per coordinator-direction; branches in parallel during PR-B1 review). After Session 3's **PR-B2** forwards per-request `influencer_id` from public-api + the `influencer_id_source` log marker shifts `env_fallback` → `request` in prod traces, **PR-B3** drops the env-var fallback + makes `request.influencer_id` required. First post-PR-A+PR-B1+PR-B2 smoke targets a non-NSFW influencer; Tara routing through OpenRouter is Phase-2 follow-up (needs OpenRouter API key on cluster + LLM-routing matrix wired).
