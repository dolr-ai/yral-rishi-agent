...
Redis client-side AUTH wiring PR #136 open + round-2 fixup pushed (this branch — Python + YAML + LOG + STATE; round-2 was test-rename only, zero functional change). DRAFT stays on through Codex re-review; **NOT I14 auto-merge eligible** (Python Settings field + `master_for` keyword argument + behavior-changing compose adds new secret mount + new external-name declaration). Coordinator confirms `yral_v2_redis_primary_password_ceeb8b19` Swarm secret exists on cluster pre-merge + drives stack re-deploy after merge — unblocks cluster smoke `POST /v1/turn` with shmeena12. **PR-D1 (influencer-directory service-build) green-lit to start in parallel** with Q1–Q5 decisions locked + archetype (γ) approved.
