...
> Updated: 2026-05-22 (Day-8 PR-A env-gate fix PR opened — flip `ENVIRONMENT` default from `production` → `staging` across 3 Session-4 service composes; same fix for public-api raised as DEP-011 for Session 3 per I9 scope-discipline; A6 production-cutover gate preserved at predicate level).
...
Day-8 PR-A env-gate fix PR open (this PR — `.yml`-only + `.md`-only behavior-changing config; cumulative line count verified pre-push via `git diff --stat main...HEAD`). **NOT I14 auto-merge eligible** (YAML change flips runtime ENVIRONMENT label across 3 services — behavior-changing config falls outside I14's narrow .md-only / test-only / lint-only / comment-only allowance); coordinator manually merges via `gh pr merge <N> --squash` after Codex APPROVE. Queued next: **PR-B1** (`RunTurnRequest` widen with optional `influencer_id` + env-var fallback for backwards-compatibility) → **influencer-directory metadata schema + RPC endpoints** (Postgres schema for `influencer_metadata` table, ETL from chat-ai's 3,941 `ai_influencers` rows, 3 endpoints for public-api per coordinator-direction; parallel or after PR-B1) → **PR-B3** (drop env-var fallback + require `influencer_id` after Session 3's PR-B2 forwards it end-to-end). First post-PR-A smoke targets a non-NSFW influencer; Tara routing through OpenRouter is Phase-2 follow-up (needs OpenRouter API key on cluster + LLM-routing matrix wired).
