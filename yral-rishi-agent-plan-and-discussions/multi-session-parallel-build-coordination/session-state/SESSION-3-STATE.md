...
> Updated: 2026-05-25 (PR #137 Redis client-side AUTH wiring still DRAFT through round-13. Wires `REDIS_PASSWORD` on public-api's 2 Redis paths (single-URL `from_url()` + Sentinel `master_for()`) per H3 + 2026-05-22 rotation. Cumulative diff vs merge-base: 12 files / +1089 / -88 / 11 commits (round-1 through round-12) + round-13 in progress; 6 J1-HOT tests in `test_health_routes.py` covering both Redis-AUTH paths + 3 validator regression tests (rejection-when-flag-on, acceptance-when-flag-on, default-flag-off-allows-credential-bearing-URL safety net). Major round arc: rounds 1-3 wired the kwargs + Codex CONCERN on test-isolation leak + B2 abbreviation scrub; rounds 4-5 D8-regen `.env.example` + restore REDIS_URL local default; rounds 6 + 11-12 ongoing B2 sweeps (`kwarg`/`positional_args`/`dev`/`prod`/`env vars`/`dir`/`cwd`/`tmp`); round-8 added passwordless-URL contract + validator; round-9 documented merge-order; round-10 preemptive `.env.local` → `env.local.fixture` rename + mktemp-copy-rename runner mirroring Session 4 PR #148 round-4; round-11 implemented feature-flag pattern (`enforce_passwordless_redis_url: bool = False`) per Codex round-9 BLOCKER 3 — supersedes the merge-order gate so this PR is now safe to merge before PR #150 + secret rotation. PR-B (#130) + PR-B1 (#131) merged 2026-05-23; PR-B2 (#141) DRAFT at round-5 holding for PR #145 merge per coordinator directive; PR #150 + Session 1 secret rotation are now Session 1 follow-ups (no longer block this PR).)
...
**2026-05-25 — PR #137 round-13 (B7 function header on `assert_exit_code` + STATE refresh per Codex round-12 verdict).** PR #137 has been iterating since 2026-05-23 across 12 rounds with the same overall feature shape (REDIS_PASSWORD wiring on both public-api Redis paths) but evolving Codex feedback: rounds 1-3 base implementation + tests + initial B2 scrub; rounds 4-5 the `.env.example` D8-regen arc; round-6 + 11-12 ongoing B2 abbreviation sweeps (`kwarg`/`positional_args`/`dev`/`prod`/`env vars`/`URI`/`dir`/`cwd`/`tmp`); round-8 the passwordless-URL validator + role-comment contract; round-9 merge-order documentation that round-11 replaced with a real mechanism (the `enforce_passwordless_redis_url: bool = False` feature flag — when OFF, validator no-ops; when Session 1 flips ON post-rotation, validator rejects credential-bearing URLs); round-10 the preemptive `.env.local` → `env.local.fixture` rename + mktemp-copy-rename test runner mirroring Session 4 PR #148 round-4; round-12 typo cleanup + final B2 scrub on the round-10 ported runner. **Round-13** (this commit) adds the explicit B7 WHAT/WHEN/WHY function header to `assert_exit_code` in `test_validate_secrets.sh` (the helper body inline-prose explained mechanics but didn't meet B7's function-header format) + refreshes this STATE snapshot which had drifted to the round-1 numbers (`6 files / +262 lines / 3 tests`) per Codex round-12 CONCERN. **Cumulative state**: 12 files / +1089 / -88 / 11 commits + round-13 (12th); 6 J1-HOT tests covering both Redis-AUTH paths + 3 validator regression tests (the new flag-off safety-net test added round-11). **Merge gate**: Codex APPROVE only — the round-11 feature flag removed the round-9 coordinator-gated sequencing dependency on PR #150 + Session 1 secret rotation (those are now a Session 1 follow-up that flips the flag TRUE). PR-B2 (#141) DRAFT at round-5 holding for PR #145 merge; PR-B (#130) + PR-B1 (#131) merged 2026-05-23.
