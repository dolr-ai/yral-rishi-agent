...
**2026-05-25 — PR #137 round-19 (test-docstring scope-fix sweep on round-17-missed `Session 1 flips` attribution + validator strengthening for empty-credential URLs per Codex round-18 verdict).** Round-19 closes 1 BLOCKER + 1 CONCERN: (BLOCKER) the round-17 production-code "Session 1 → Session 3" attribution sweep missed the test docstrings in `tests/contract/test_health_routes.py` — 3 docstring occurrences corrected (the round-8 rejection test's WHEN block + the flag-off safety-net test's WHY + WHEN blocks) to say "Session 3 flips the compose default via a small follow-up PR (after Session 1 lands PR #150 + secret rotation + confirms passwordless state)" consistent with the I9 ownership split; (CONCERN — validator strengthening) Codex caught that the round-8 validator's `if parsed.username or parsed.password:` check let empty-credential URL forms slip through because empty strings are falsy in Python — `redis://:@host:6379/0` and `redis://@host:6379/0` both have empty/None credentials but redis-py's URL parser still interprets the `@` separator as a credential-bearing URL, defeating the passwordless-URL contract. Round-19 changes the condition to `if "@" in (parsed.netloc or ""):` which catches ALL credential-separator forms (`user:pass@`, `:pass@`, `user:@`, `:@`, `@`); + adds a new regression test `test_redis_url_with_empty_credentials_is_rejected_when_flag_is_on` that iterates over 4 empty-or-partial-credential shapes and asserts each raises ValidationError when the flag is on; + updates the validator's docstring + the `urllib.parse.urlparse` import role-comment + the inline rejection-branch comment to explain the round-19 strengthening. **Cumulative state**: 12 files / +1212 / -89 / 18 commits + round-19 (19th); 6 J1-HOT tests in `test_health_routes.py` covering both Redis-AUTH paths + **4 validator regression tests** (rejection-when-flag-on, acceptance-when-flag-on, empty-credential-rejection-when-flag-on, default-flag-off-allows-credential-bearing-URL safety net); validate-secrets.sh 5/5. **Merge gate**: Codex APPROVE only — the round-11 feature flag (`enforce_passwordless_redis_url: bool = False`, default OFF) removed the coordinator-gated sequencing dependency on PR #150 + Session 1 secret rotation. Activation path: Session 1 confirms secret state post-rotation; **Session 3** flips the public-api compose `${ENFORCE_PASSWORDLESS_REDIS_URL:-false}` → `:-true` in a small follow-up PR. PR-B2 (#141) UNBLOCKED by PR #145 merge (2026-05-25 10:13 UTC; round-6 update queued post-#137-close-out — STATE "exception" framing → ratified-architecture reference + Gate A1 integration-test acceptance criterion); PR-B (#130) + PR-B1 (#131) merged 2026-05-23.
