...
def test_migration_044_table_shape():
    ...
    Types reflect squawk compliance: id uses IDENTITY (not BIGSERIAL),
    counters are bigint (squawk's prefer-bigint-over-int rule). Python
    ints map to bigint via asyncpg without code change."""
    src = _read("migrations/044_reply_evaluations.sql")
    assert "CREATE TABLE IF NOT EXISTS reply_evaluations" in src
    # squawk: statement_timeout cap on the migration
    assert "SET statement_timeout" in src
    # squawk: IDENTITY instead of BIGSERIAL
    assert "id              bigint generated by default as identity primary key" in src
    for col in (
