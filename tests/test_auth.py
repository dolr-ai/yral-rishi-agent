...
def test_expired_token():
    from auth import get_current_user
    from fastapi import HTTPException
    token = _make_jwt({"sub": "user1", "iss": "https://auth.yral.com", "exp": int(time.time()) - 3600})
    with pytest.raises(HTTPException) as exc_info:
...
def test_untrusted_issuer():
    from auth import get_current_user
    from fastapi import HTTPException
    token = _make_jwt({"sub": "user1", "iss": "https://evil.com", "exp": int(time.time()) + 3600})
    with pytest.raises(HTTPException) as exc_info:
        get_current_user(_make_request({"Authorization": f"Bearer {token}"}))
    assert exc_info.value.status_code == 401
...
def test_valid_token():
    from auth import get_current_user
    token = _make_jwt({"sub": "user-principal-123", "iss": "https://auth.yral.com", "exp": int(time.time()) + 3600})
...
def test_lowercase_bearer():
    from auth import get_current_user
    token = _make_jwt({"sub": "user-456", "iss": "https://auth.dolr.ai", "exp": int(time.time()) + 3600})
    user_id = get_current_user(_make_request({"Authorization": f"bearer {token}"}))
