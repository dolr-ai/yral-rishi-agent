...
def test_cost_alerts_module_exists():
    """services.ops.cost_alerts must define the two checks the loop wires to."""
    src = (REPO / "app" / "services" / "ops" / "cost_alerts.py").read_text()
...
def test_no_email_digest_section_per_rishi_2026_06_09():
    ...
    src = (REPO / "app" / "services" / "ops" / "cost_alerts.py").read_text()
    digest = (REPO / "app" / "services" / "ops" / "email_digest.py").read_text()
...
def test_main_wires_cost_alerts_task():
    ...
    assert "from services.ops.cost_alerts import cost_alerts_loop" in main
