...
# `app/redis_client.py`:
#
#   1. verify_deployed_environment_sentinel_or_die() raises RuntimeError when
...
#      propagate cleanly).
#   2. verify_deployed_environment_sentinel_or_die() raises RuntimeError when
...
#   3. verify_deployed_environment_sentinel_or_die() does NOT raise when
...
# call site.
from app.redis_client import init_redis, verify_deployed_environment_sentinel_or_die


# ===========================================================================
# verify_deployed_environment_sentinel_or_die() — C11 production-fail-closed gate
# ===========================================================================


def test_sentinel_disabled_in_production_raises_runtime_error(
    monkeypatch,
):
    """Production env + REDIS_SENTINEL_ENABLED=false → RuntimeError.

    WHAT: monkeypatches ENVIRONMENT=production + REDIS_SENTINEL_ENABLED=
          false, calls verify_deployed_environment_sentinel_or_die, asserts the
...
def test_sentinel_disabled_in_staging_raises_runtime_error(
    monkeypatch,
):
...
def test_sentinel_disabled_in_local_environment_does_not_raise(
    monkeypatch,
):
    """Local env + REDIS_SENTINEL_ENABLED=false → no raise (local OK).

    WHAT: monkeypatches ENVIRONMENT=local + REDIS_SENTINEL_ENABLED=
          false, calls verify_deployed_environment_sentinel_or_die, asserts
...
async def test_missing_sentinel_master_name_raises_runtime_error(
    monkeypatch,
):
    """Sentinel enabled + sentinel_master_name missing → RuntimeError.

    WHAT: monkeypatches REDIS_SENTINEL_ENABLED=true +
          ENVIRONMENT=staging (so verify_deployed_environment_sentinel_or_die
...
async def test_missing_sentinel_hosts_raises_runtime_error(
    monkeypatch,
):
