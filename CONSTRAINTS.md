...
| C9 | **Datacenter placement**: rishi-1/2/3/4 = Falkenstein (FSN1, confirmed by IP range). rishi-5 ambiguous. rishi-6 likely Nuremberg (NBG1). Verify via `hcloud server describe` or Saikat. If rishi-6 is cross-DC: Patroni sync replica stays in FSN1 (rishi-4+5); rishi-6 runs async-only | 🟡 | V2_TEMPLATE_AND_CLUSTER_PLAN §5 + §10 | Day-0 verification during provisioning; adjust Patroni topology based on result |
...
| E4 | **No cost controls until product-market fit.** Spend whatever needed on LLMs and infra. Per-turn cost TRACKED in Langfuse for visibility; NOT gated | 🔒 | `feedback_no_cost_controls_until_product_market_fit.md`; Rishi 2026-04-23 | Langfuse tracks cost per turn; no blocking circuit breaker (this supersedes the "cost-cap circuit breaker" row in V2_TEMPLATE_AND_CLUSTER_PLAN §7.3 — to be reconciled) |
...
| F3 | **Schema-per-service on ONE shared Patroni cluster** on rishi-4/5/6 (not per-service Patroni). Tenant SQL creates schema + role + GRANTs + connection cap at bootstrap | 🟡 | V2_TEMPLATE_AND_CLUSTER_PLAN §7.2 — "consciously accepted 2026-04-23" — but needs Rishi explicit confirm | Service bootstrap script; each service sees only its own schema |
| F4 | **Staging environment from day 1**, shared infra (same Patroni cluster, Redis, Langfuse), namespace separation (`staging_chat_ai_v2_*` schemas, `staging:` Redis prefix, `environment=staging` Sentry/Langfuse tag). 1-replica per service at 50% prod resources | 🟡 | V2_TEMPLATE_AND_CLUSTER_PLAN §5 | Every service has staging deploy; manual "promote to prod" button in GitHub Actions |
...
| F7 | **Separate cluster-bootstrap repo** (`yral-rishi-chat-ai-v2-cluster-bootstrap`) for node bootstrap scripts, systemd units, UFW rules, `cluster.hosts.yaml`, `services.yaml`. One repo per cluster, not per service | 🟡 | V2_TEMPLATE_AND_CLUSTER_PLAN §12 | Separate repo created; template and services never own cluster-level config |
...
## Appendix A — Known Conflicts to Reconcile

These are places where two sources disagree and we need Rishi's call:

1. **Cost-cap circuit breaker vs no-cost-ceiling.** V2_TEMPLATE_AND_CLUSTER_PLAN §7.3 lists "Cost-cap circuit breaker (added 2026-04-23)" as a template feature — but Rishi later (same day) said **"no cost controls until product-market fit"** per `feedback_no_cost_controls_until_product_market_fit.md`. Current reconciliation in row E4: **track cost per turn in Langfuse, but DO NOT gate or circuit-break.** Needs Rishi confirm.
2. **Schema-per-service-on-one-cluster (row F3) vs per-service Patroni.** Other doc locks the shared-cluster approach; my earlier draft in README was ambiguous. Needs Rishi explicit OK on "all 13 services have correlated outage risk if shared Patroni goes down" — mitigated by HA + chaos tests + 3-layer backup.
3. **Staging shares expensive infra (row F4) vs isolated staging.** Shared is cheaper and simpler; isolated is safer. V2_TEMPLATE_AND_CLUSTER_PLAN locks shared. Needs Rishi confirm.
4. **Cluster-bootstrap repo (row F7) as separate.** Clean, but adds a repo. Alternative: fold into template repo. V2_TEMPLATE_AND_CLUSTER_PLAN proposes separate.
