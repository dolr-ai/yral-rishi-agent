...
| F15 | **Service build order (Q12, Rishi 2026-04-23):** memory + influencer-directory prioritized earlier to ensure v2 has depth + preserved influencers from early on. Proposed slicing: **Slice 1** = `public-api` + `conversation-turn-orchestrator` (MVP turn, Weeks 4-6); **Slice 2** = `user-memory-service` + `influencer-and-profile-directory` (Weeks 7-10, promoted earlier per Q12); **Slice 3** = `soul-file-library` + `content-safety-and-moderation` (Weeks 11-13); **Slice 4** = `creator-studio` + `events-and-analytics` (Weeks 14-16); **Slice 5+** = proactive-scheduler, media, skill-runtime, payments, meta-advisor | 🟡 | Rishi 2026-04-23 (Q12 option c) — needs Rishi confirm the exact slicing | Each slice has entry + exit criteria in the v2 plan roadmap |
