...
| A10 | **Tara stays on OpenRouter.** She's the best bot YRAL has. Preserved in the llm-client abstraction via a per-influencer-id routing rule. Other bots use Gemini by default (or Claude for crisis turns, or OpenRouter for NSFW per existing behavior) | 🔒 | Rishi 2026-04-23 evening | Routing matrix in `llm-routing-matrix-with-tara-openrouter-preservation/llm-routing-per-influencer.md`. DB-backed config table so adding/changing rules = one SQL update, no code change |
