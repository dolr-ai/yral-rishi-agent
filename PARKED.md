...
| **`production` environment has no branch policy** | 9 workflows hold `DEPLOY_SSH_KEY`, which reaches every prod node, and `workflow_dispatch` lets a caller pick any ref — so a branch can run key-holding code. 8 of them declare `environment: production`, but that environment has **no protection rules and no deployment branch policy**, so the declaration currently gates nothing. Setting the policy to protected branches fixes all 9 at once. It is a GitHub settings change, and it would stop dispatching drills from a branch — Rishi's call. Raised by Codex on #529 |
| **CI trusts SSH host keys on first sight** | All 9 SSH workflows do `ssh-keyscan >> known_hosts` immediately before connecting, which authenticates nothing: a network MITM could impersonate a manager. Worst case for the read-only drift check is false reassurance; for `deploy.yml` and `rollback.yml` it is worse. Fixing one workflow is cosmetic — the fix is pinned host keys in a secret, applied across all 9. Raised by Codex on #529 |
