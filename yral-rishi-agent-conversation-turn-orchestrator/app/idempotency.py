...
async def init_redis() -> None:
    ...
    if settings.redis_sentinel_enabled:
        ...
        # The `or None` guard normalises empty-string → None so
        # redis-py skips the AUTH frame entirely on the local path
        # (some redis-py versions treat password="" differently from
        # password=None — None is the unambiguous "skip AUTH" signal).
