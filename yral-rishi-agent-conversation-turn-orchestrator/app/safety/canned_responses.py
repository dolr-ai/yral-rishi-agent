...
#
# Each returns a `MessageDto`-shaped DICT (NOT the Pydantic model) so a
...
# to `app/models/turn.py::MessageDto` so mobile clients consuming the
# `ApiResponse<MessageDto>` envelope through Session 3's public-api see
...
# directly; constructing a Pydantic `MessageDto`, then calling
...
# (`test_safety_stack.py` asserts every key MessageDto requires).
#
...
# ===========================================================================
# Shared builder — one place that knows the MessageDto field set so
# all three response builders stay in lockstep with `models/turn.py`.
# ===========================================================================


def _canned_dto(conversation_id: str, content: str) -> dict:
    """Return a MessageDto-shaped dict for a safety-blocked reply.

    WHAT: assembles the 8 required MessageDto fields with the safety
    ...
    WHY:  one builder means a future MessageDto schema bump only
...
def prompt_injection_blocked(conversation_id: str) -> dict:
    ...
    WHAT: the MessageDto a user sees when their input matches the
...
def crisis_response(conversation_id: str) -> dict:
    ...
    WHAT: the MessageDto a user sees when their input contains
...
def nsfw_blocked(conversation_id: str) -> dict:
    ...
    WHAT: the MessageDto a user sees when the handler's RESPONSE
...
#   ../models/turn.py          — MessageDto schema (shape these dicts mirror)
