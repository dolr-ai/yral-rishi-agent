...
# (H5/H4/A10) wraps this route as middleware without touching the
# route signature.
app.include_router(run_turn_router)

# -------------------------------------------------------------------
# Day-6 safety stack — H5 → H4 → A10 → handler (REQUEST flow).
# -------------------------------------------------------------------
...
#   RequestId → H5 → H4 → A10 → run_turn handler
# Net wire-level RESPONSE flow (LIFO unwinds):
#   handler → A10 → H4 → H5 → RequestId
#
