...
| `services/llm_registry.py` | **Unchanged.** Skilled influencers (Kareena, future english_coach, daily_briefing, travel_advisor, real_estate_advisor, …) flow through the existing `user_chat_main` process exactly like Tara or any non-skilled influencer. The skill identity lives in Soul File composition (skill prompt block + `user_skill_state` plan layer) — **not** in routing. Adding a new skill is a one-line `SKILLS` dict entry. **Zero** registry changes. **Zero** new processes. If per-skill cost tracking is ever needed, the right move is a `skill_slug` tag column on `llm_costs` (one query → cost per skill), not a process split (which would multiply routing surface every time a new vertical ships). |
