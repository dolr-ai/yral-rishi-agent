# ===========================================================================
# WAVE 1 REFERENCE TEMPLATE — not a live conftest. Do not let pytest collect
# this file (it lives under docs/ precisely so it is out of the test path).
#
# WHY THIS FILE EXISTS
# Rescued from the deleted archive/ (soul-file-library, the richest of the
# archived conftests) in Wave 0 PR2. It is the reference pattern for the
# testcontainers-Postgres safety net that Wave 1 builds: a session-scoped
# ephemeral Postgres, a real migration applied against it, per-function
# TRUNCATE-and-reseed isolation, and an in-process httpx/ASGI client so the
# app and the test share ONE asyncpg pool on ONE event loop.
#
# WHAT MUST BE ADAPTED FOR v2 (this is NOT copy-paste ready)
#   1. MIGRATIONS: this template shells out to `alembic upgrade head`. v2 has
#      no Alembic — it applies numbered SQL files under migrations/ via raw
#      asyncpg (see migrations/ + app/database.py). Wave 1 replaces the
#      run_alembic_upgrade fixture with one that runs the numbered .sql files
#      in order against the container.
#   2. CONNECTION ENV VAR: v2 reads DATABASE_URL (a multi-host asyncpg DSN),
#      not POSTGRES_CONNECTION_STRING_SOUL_FILE_LIBRARY. Point the container's
#      single-host URL at DATABASE_URL for the test session.
#   3. POOL GLOBAL: v2's pool lives in app/database.py (get_pool / lazy
#      _pool). Confirm the exact global name before binding it (app_pool_bound).
#   4. SEED DATA: the _truncate_and_reseed block below is soul-file-specific
#      (soul_file_layers). Replace with the tables Wave 1's first DB tests
#      actually touch.
#
# The DESIGN is what to keep: session-scoped container + migrate-once +
# per-function reset + single-loop ASGI client. See docs/cleanup-plan for the
# Wave 1 plan that consumes this.
# ===========================================================================
