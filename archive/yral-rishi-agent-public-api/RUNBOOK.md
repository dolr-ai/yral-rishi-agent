# RUNBOOK — yral-rishi-agent-public-api
...
2. **Manual** — re-run the deploy workflow with the last-known-good image tag (set as a workflow input). Tag format: `ghcr.io/dolr-ai/yral-rishi-agent-public-api:<prior-git-sha>`.
...
| Tail logs | Logs ship to Loki via the structured-logging pipeline. Query in Grafana with `{service="yral-rishi-agent-public-api"}`. |
...
3. Common causes:
   - Missing Swarm secret (`REDIS_URL`, `SENTRY_DSN`, `LANGFUSE_PUBLIC_KEY`, `LANGFUSE_SECRET_KEY`) → check `docker secret ls` on a manager.
   - Redis Sentinel quorum unreachable → check Session 1's stateful-core stack on rishi-4/5/6.
...
3. Check Sentry performance tab (`sentry.rishi.yral.com`) — filter by `service=yral-rishi-agent-public-api`.
