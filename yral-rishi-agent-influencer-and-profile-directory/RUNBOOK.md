# RUNBOOK — yral-rishi-agent-influencer-and-profile-directory
...
2. **Manual** — re-run the deploy workflow with the last-known-good image tag (set as a workflow input). Tag format: `ghcr.io/dolr-ai/yral-rishi-agent-influencer-and-profile-directory:<prior-git-sha>`.
...
| Tail logs | Logs ship to Loki via the structured-logging pipeline. Query in Grafana with `{service="yral-rishi-agent-influencer-and-profile-directory"}`. |
...
3. Check Sentry performance tab (`sentry.rishi.yral.com`) — filter by `service=yral-rishi-agent-influencer-and-profile-directory`.
