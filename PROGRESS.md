...
| 21γ.P22 | **BUG: AI Influencer name split-brain across yral-metadata-server and v2.** Discovered 2026-06-24 while applying `nutrition_coach` skill to Neha's bot. Mobile "My Profiles" page reads the AI Influencer's title from `AccountInfo.displayName` — a computed property off `username`, fetched from **yral-metadata-server** (metadata.yral.com). The chat header + system prompt read from **v2 `ai_influencers.display_name`**. These can diverge cleanly: Neha's profile shows "Nutritionist" (metadata) while v2 has `display_name = "Dr. Anya Sharma"` (the AI character name set at create time). Source: `shared/core/.../AccountInfo.kt` — `val displayName: String get() = resolveUsername(username, userPrincipal) ?: userPrincipal`. Functional impact: cosmetic-only today (skill + chat both work fine pointed at the right v2 row), but UX is confusing — same bot has two names depending on screen. Fix options: (a) mobile sends `username` to v2 at create-time and v2 keeps `display_name` in sync, (b) mobile reads v2's `display_name` everywhere, (c) v2 fetches `username` from metadata at chat-time. Pick when we have bandwidth + a clearer product call on which name wins. | 0.5-1 day investigation + fix |
| 21γ.P23 | **Watchdog cannot report a Sentry outage — external dead-man's switch.** 2026-08-08: Sentry's embedded Docker DNS registry was evicted on rishi-3, every container stayed "Up" but stopped resolving siblings, and Sentry dropped **every event for ~45h** with nothing paging. The watchdog was structurally blind three ways: `node.role == manager` excludes rishi-3, its DNS check resolves *Swarm overlay aliases* (not compose bridge networks), and — the load-bearing one — **all three checks alert through `sentry_sdk`, so none can report Sentry being down.** Making the watchdog global fixes none of that. Fix: `watchdog/heartbeat.py` pings an external dead-man's switch *while* the fleet's public surfaces answer 2xx, so silence is the alarm — also covering "watchdog dead" and "Swarm gone". Vendor-neutral + unset by default, so inert until an operator opts in. | 🔄 In PR — code + 10 tests green. **Blocked on Rishi's call:** external provider (independent, the property that failed) vs. an owned endpoint (`feedback_extend_owned_services_over_vendor_tools`). Deploy is a `docker stack deploy` after merge. | 0.5 | this PR |
| 21γ.P24 | **Backport the rishi-3 Sentry network fix to the `yral-rishi-sentry` repo.** The 2026-08-08 recovery live-edited `docker-compose.override.yml` on rishi-3: nginx now attaches to `yral-v2-public-web` declaratively with `ipv4_address: 10.0.1.11` pinned + a `sentry-nginx` alias, replacing the deleted `sentry-web` external network that the 2026-08-01 consolidation orphaned. Live fix is verified working but exists only on the host — a `scripts/install.sh` re-run or repo redeploy would revert it. Also worth folding in: point the edge Caddyfile at the `sentry-nginx` alias so `sentry.rishi.yral.com` stops depending on a hardcoded `10.0.1.11:80`. | ⏳ Pending — live fix holding, backup at `~/override.yml.bak-20260808` | 0.5 | — |
...
| # | Item | Status | Est days | PR |
|---|---|---|---|---|
| 27.0 | **ARCHITECTURE DECISION (Rishi 2026-08-12): one shared database for the POC.** Deliberately overrides the July contract's Level-2 write isolation (`docs/amorae-v2-contract-2026-07-01.md`) — adult + mainstream both served from the same DB while we find out whether the product works. Split later only if it proves out. The `/api/v1/spicy/*` contract endpoints still exist and still work; they're just no longer the only channel | ✅ Decided — do NOT re-litigate as a violation | — | — |
| 27.1 | **`surface` column + opt-in catalogue filter.** `surface TEXT NOT NULL DEFAULT 'mobile'` + CHECK + index; `app/services/surface.py`; `?surface=` on `GET /api/v1/influencers` (absent = unfiltered, unknown = 400); `surface` in the response | ✅ Done — merged #481, migration 052 applied, deployed. **Verified live in prod:** `?surface=web` → total 1 (Tara); no param → total 3806 unchanged; `?surface=wbe` → 400 | 0.5 | #481 |
| 27.2 | **Flag Tara `web` vs `both`** (`qi6gd-…-5qe`, `taaarraaah`, the ONE active `is_nsfw` row) | ✅ Done 2026-08-12 — Rishi chose **`both`**. `UPDATE 1` on the leader, guarded by exact id AND `is_nsfw`. Distribution now 4,084 mobile / 1 both. Chosen because `/api/v1/influencers` never filtered `is_nsfw`, so she was ALREADY on mobile — `web` would have removed a 54k-conv bot. **Later options Rishi named:** drop Tara from mobile entirely, or split into two versions of the same influencer (clean in-app, unclean on web) | 0.1 | — |
...
**Hardened 2026-08-12 (#465, in repo):** asyncpg's own multi-host connect raises on a host stuck "starting up" instead of failing over — which took v2 fully down on 2026-08-10 when a recovering node sat first in `DATABASE_URL`. `app/database.py` now iterates the hosts itself on `CannotConnectNowError`, skipping starting-up/read-only hosts to reach the leader. Deployed + validated live.
...
**MERGED (#457) 2026-08-12, NOT yet live.** `shm_size: 2g` + sequenced `update_config { parallelism:1, order:stop-first, delay:60s, monitor:90s }` on all 3 patroni services in `bootstrap/scripts/patroni-stack.yml`. Rebased past #482 so shm_size and the SPILO trust rule coexist in one redeploy. **Still shows 64 MB on all nodes (verified 2026-08-12)** — deploy.yml ships the app, not the patroni stack, so this needs a separate `docker stack deploy` in a planned window (leader failover). **Bridge mitigation planned:** `ALTER SYSTEM SET max_parallel_workers_per_gather = 0` (needs explicit Rishi authorization on the SQL — auto-mode blocked). Feed_ranker + video_ideas run single-threaded until the redeploy.

**Action:** schedule the planned patroni redeploy window (est ~15-20 min end-to-end with sequenced restart via `update_config`), verify each node comes up with `shm_size=2G` in inspect + the SPILO trust rule survives. Post-fix: revert `max_parallel_workers_per_gather` to default 2.
...
| Register the draft against the **bot**, not the human owner | 🔄 Encoding fixed in PR; blocked on a stale `ext_ai_account_ids` claim | #498, this PR |
| Mobile: refresh the token after `create_ai_account` so the new bot is in `ext_ai_account_ids` | ⏳ Not started — blocks bot-owned drafts *and* the profile dropdown | — |
