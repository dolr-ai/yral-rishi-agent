...
| 🔄 21γ.P29-PREREQ | **B6 COST CIRCUIT BREAKER NEVER SHIPPED — H2 closure premise was wrong.** Discovered 2026-06-16 morning by developer session researching `21γ.P29` drill plan. Evidence: PR #289 closed unmerged. `21α.B6a` still ⏳ Pending. `grep -r 'cost_breaker\|CostCeilingExceeded\|circuit_open' app/` → ZERO matches. The 2026-06-14 H2 WON'T FIX decision rested on B6+H11+19.1 being a live 3-layer trio (ENFORCE+DETECT+ENFORCE). **Actual state: 2-of-3 — H11 DETECTS, 19.1 ENFORCES rate-but-not-cost, B6 (the actual cost-ENFORCE layer) doesn't exist.** At 100% prod this means: bypass user drives cost past H11 threshold → Sentry alert fires → nothing auto-stops further calls → cost runaway until manual SSH intervention. **DECISION 2026-06-16 (Rishi): Option X — ship B6 this week as new top priority.** Design doc shipped via **PR #397** (`docs/b6-cost-circuit-breaker-design-2026-06-16.md`): hybrid threshold (per-user-daily + global-hourly), Postgres `llm_costs` aggregation with 10s Redis cache, check at `llm_registry._do_complete()` chokepoint (NOT chat.py — protects background paths too), `circuit_breaker_config` hot-edit table + 60s cache (rate_limiter pattern), shadow mode ≥7 days, mobile-safe 503 + Retry-After response (NOT 402 — that was H2's mistake; NOT 429 — wrong semantic). 5 hard properties baked in: DEFAULT OPEN + FAIL OPEN ON ERRORS + HOT-EDIT KILL SWITCH + SHADOW MODE FIRST + MOBILE-SAFE RESPONSE SHAPE. **🔄 In design — PR #397 open for Rishi review of 3 open questions (per-user $ threshold, mobile response shape mobile-expert confirmation, shadow-mode duration) before implementation lands tomorrow.** `21α.B6` stays Pending until implementation merges. | 🔄 In design (PR #397), implementation 1-1.5 days after Rishi green-lights doc |
