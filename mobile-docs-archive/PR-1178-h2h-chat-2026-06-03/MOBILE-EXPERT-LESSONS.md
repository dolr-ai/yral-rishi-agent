...
## P8 — Read the exception, not just the symptom. A broad `except Exception` is a structural amplifier of opaque failure.

**The trap:** when N distinct upstream causes collapse to a single observable string (the user-visible symptom), code-reading the surrounding stack produces hypotheses **ordered by plausibility, not by truth**. You pick the most plausible hypothesis, ship a fix, the fix is real, you feel progress, the symptom re-fires. Pick the next-most-plausible, ship, feel progress, symptom re-fires. You can do this 2-3 rounds before noticing the meta-pattern: **the diagnosis was never observational, it was always pattern-matched.**

The structural amplifier of this trap is the broad `except Exception` (or `try/finally swallow`, or `runCatching { ... }.getOrNull()`, or any pattern that converts a distinct exception class into a generic None / null / empty / fallback string). The amplifier collapses N causes to one symptom — at which point code-reading IS the only diagnostic move available unless you go find the actual exception text another way. And the actual exception text is usually one log line + one timestamp away.

**The lesson:** when you see the *same* symptom recur after a hypothesis-based fix, stop. The next "plausible cause" you can think of by reading code is *also* a hypothesis. Hypotheses on top of hypotheses do not converge on truth. The diagnostic move is to find where the exception is being silently caught, find the timestamp of the latest failed reproduction, pull the actual `e` repr from logs (Sentry, `docker service logs`, file logger — wherever it actually goes), and read it. Runtime evidence beats N rounds of code-reading.

**Worst form of the trap — the *plausible-and-related* hypothesis:** the proposed fix has a plausible chain of causation AND addresses a real bug in the surrounding code, but is not the root cause of the *currently observed* failure. It ships green. CI passes. Deploy succeeds. The fix is real progress on something — just not on the thing you were trying to fix. The next test fails identically to the previous one. From the outside, this looks like "the fix didn't deploy." From the inside, the deploy did happen, the fix did land, the original bug is still there. Two or three iterations of this and the user starts asking why you keep failing — and they're right to ask.

**Defense:**

1. **Demand the exception text before proposing a fix when the symptom is generic.** `"[Audio message - transcription unavailable]"` is a generic symptom — the same string fires for storage_key resolution failures, fetch failures, Gemini rejection, registry mis-wiring, and at least 3-4 other distinct causes. The string itself tells you nothing about which. Ask for the exception text from the catch site before ordering hypotheses.

2. **When code-reading does produce a hypothesis, label it explicitly as a hypothesis, not a diagnosis.** Words matter here. "I think this is the bug" ≠ "I observed this is the bug." Reviewers (and you, on the next loop) need to know which kind of statement you're making.

3. **Track the *recurrence pattern*, not just the fix queue.** If you've shipped 2 fixes for the same observable and the third repro is identical, you are pattern-matching, not diagnosing. Stop, get the runtime evidence, *then* propose the third fix.

4. **Watch for broad excepts in code you're trying to debug.** `except Exception: return None` or `runCatching { ... }.getOrNull()` are structural anti-amplifiers. When you spot one in a stack you're triaging, the FIRST move is to widen the catch to log `e` repr (with stack trace if possible) — even if that takes a tiny PR — and re-run. The widened log buys you orders-of-magnitude more diagnostic signal than another round of code-reading.

5. **Plausible-and-related hypotheses are the worst class. Be especially skeptical when your "fix" addresses a real bug nearby but the symptom didn't change after deploy.** That's the strongest possible signal that the symptom is hiding a different (still un-fixed) cause behind a broad except.

**Bug-trail context, 2026-06-03:** Phase 1.7b audio mic-recording feature. End-user symptom: voice messages sent fine (file persists, AI replies) but the user-message content showed `"[Audio message - transcription unavailable]"`. I diagnosed three times by code-read and shipped two backend briefs that both became merged PRs (#254 — `chat.py` storage_key resolution; #255 — `gemini.transcribe` image-fetcher fork). Both were real bugs in the surrounding code. Neither was the root cause of the observed failure — the third reproduction failed identically. The amplifier was the broad `except Exception as e: logger.error(...); return None` at `ai_client.py:825-826` collapsing every upstream cause into the same "unavailable" fallback. The unblock was a single `docker service logs ... | grep "Audio transcription failed"` — one shell command with the actual `e` repr that I had been hypothesizing about for three cycles. Rishi pointed this out plainly mid-cycle, which is why this lesson exists.
